...
class MyHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def route(self):
        ...
        for matcher,handler in self.routes:
            ...
            if match:
                try:
                    return self.call_handler_cursor(sql_connection, handler, match)
                except mysql.connector.errors.OperationalError:
                    sql_connection=obtain_sql_connection()
                    return self.call_handler_cursor(sql_connection, handler, match)
        return None

    def call_handler_cursor(self, sql_connection, handler, match):
        cursor = sql_connection.cursor(buffered=True, dictionary=True)
        res = handler(match, sql_connection, cursor)
        cursor.close()
        return res
...
##        self.register_route(r'^/api/pages$',self.get_pages)
    ...
    def no_content(self, match, sql_connection, cursor):
